        ...
        <div class="col-md-8">
            <div class="card text-dark">
                ...
                <div class="card-header">
                    <h3 class="card-title">Add money to {{$customer->fname}} {{$customer->lname}}</h3>
                </div>
                <div class="card-body">
                    <p class="card-text">IBAN: {{$customer->account_code}}</p>
                    <p class="card-text">balance: {{$customer->balance}} eur.</p>
                    <form action="{{route('customers-updateAdd', $customer)}}" method="post">
                        <div class="mb-3">
                            {{-- {{dump(Session::has('no'))}}
                            {{var_dump($errors)}}
                            {{dump(old('balance'))}} --}}

                            <label class="form-label">add here</label>
